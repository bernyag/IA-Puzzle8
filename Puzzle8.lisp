...
(defun left (current zeroPos)
  (let ((aux (copy-list current)))
    (rotatef (elt current zeroPos) (elt current (- zeroPos 1)))
    current
  )
)
...
(defun getTotalCost (current)
    (cond 
        ((null current))
        (t (incf varTotalCost (getCost  (- 9 (length current)) (car current) ) ) (getTotalCost (cdr current)))
    )
  varTotalCost
)
...
(defun manhattan (current nodeNumber father level cost direction)
  ...
  (if (and (not (or (= (position 0 current) 0) (= (position 0 current) 3) (= (position 0 current) 6))) (<= (position 0 current) 8))
    (setq varTotalCost 0) 
    (setq leftCost (getTotalCost (left current (position 0 current))) ) ; TODO: arreglar a que regrese un número
